...
var commonCallBack = function(jdata){
	alertDialog(jdata.message,function(){
			...
			var _tmp_d = dialog({
				...
				ok: function () {
					if(typeof(_insertId)!='undefined'){
                       	window.location.href = '/builder?id='+json.insertId;
					}else{
						window.location.reload();
					}
				}
...
var deleteSubject = function (subjectId) {
	...
	var d = dialog({
	    ...
	    width:200,
	    ok: function () {
	    	ajaxRun('/builder/delete',data,commonCallBack);
	    },
	    cancelValue: '取消',
	    cancel: function () {
...
var setBuildPage = function(subjectId){
	...
	window.setBuildPageCallBack = function(jdata){
		aceeditor_2.setValue(jdata.message);
	}
